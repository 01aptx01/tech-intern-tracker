"use client";
import { Pie, PieChart, ResponsiveContainer, Tooltip, Bar, BarChart, XAxis, YAxis } from "recharts";
...
export function StatusCharts({ records }: { records: CompanyRecord[] }) {
  const statuses = announcementStatuses.map((name, index) => ({ name, value: records.filter((record) => record.announcementStatus === name).length, fill: colors[index] }));
  const roles = roleCategoryCounts(records);
  return <section className="chart-grid" aria-label="กราฟภาพรวม">
    <article className="panel chart-card"><div className="panel-heading"><div><p className="eyebrow">ANNOUNCEMENT</p><h2>สถานะประกาศ</h2></div></div><div className="chart-body"><ResponsiveContainer width="100%" height={220}><PieChart><Pie data={statuses} dataKey="value" nameKey="name" innerRadius={55} outerRadius={82} paddingAngle={2} /><Tooltip /></PieChart></ResponsiveContainer><ul className="chart-legend">{statuses.map((item, index) => <li key={item.name}><span style={{ background: colors[index] }} />{item.name}<b>{item.value}</b></li>)}</ul></div><p className="sr-only">{statuses.map((item) => `${item.name} ${item.value} บริษัท`).join(", ")}</p></article>
