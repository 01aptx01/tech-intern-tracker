...
export function DeadlinePanel({
  ...
  const urgent = records
    ...
    .filter((record) => {
      const days = daysUntil(record.applicationDeadline);
    ...
    <section className="panel deadline-panel">
      <div className="panel-heading">
        <div>
          ...
          <h2>
            <CalendarClock size={18} /> Deadline ที่ควรจัดการก่อน
          </h2>
        </div>
        <span className="count-chip">{urgent.length} รายการ</span>
      </div>
      {urgent.length ? (
          ...
          {urgent.map((record) => (
            <button key={record.id} onClick={() => onOpen(record)}>
              <span>
                ...
                <small>{record.applicationWindow || 'เปิดรับสมัคร'}</small>
              </span>
        ...
        <p className="empty-copy">ไม่มี deadline ภายใน 30 วัน</p>
      )}
    </section>
  );
}
