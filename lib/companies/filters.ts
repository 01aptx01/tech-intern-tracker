...
function includesAny(actual: string | null, selected: string[]) {
  return (
    selected.length === 0 || (actual !== null && selected.includes(actual))
  );
}
...
export function filterCompanies(
  ...
  return records.filter((record) => {
    ...
    if (!includesAny(record.announcementStatus, filters.announcementStatuses))
      return false;
    if (!includesAny(record.userStatus, filters.userStatuses)) return false;
    if (!includesAny(record.evidenceLevel, filters.evidenceLevels))
      return false;
    if (
      filters.techRoles.length &&
      !filters.techRoles.some((role) => record.techRoles.includes(role))
    )
      return false;
    if (
      filters.programTypes.length &&
      !filters.programTypes.some((type) => record.programTypes.includes(type))
    )
      return false;
    if (
      filters.locations.length &&
      !filters.locations.includes(record.thailandLocation ?? '')
    )
      return false;
    if (
      filters.workModes.length &&
      !filters.workModes.includes(record.workMode ?? '')
    )
      return false;
    if (filters.deadline !== 'all') {
      ...
      if (filters.deadline === 'overdue' && (days === null || days >= 0))
        return false;
      if (filters.deadline === '14' && (days === null || days < 0 || days > 14))
        return false;
      if (filters.deadline === '30' && (days === null || days < 0 || days > 30))
        return false;
    }
    return true;
  });
...
export function sortCompanies(
  ...
  return [...records].sort((a, b) => {
    if (key === 'default') {
      const byStatus =
        statusOrder[a.announcementStatus] - statusOrder[b.announcementStatus];
    ...
    const value = (record: CompanyRecord) => {
      ...
      if (key === 'applicationDeadline')
        return record.applicationDeadline ?? '9999-12-31';
