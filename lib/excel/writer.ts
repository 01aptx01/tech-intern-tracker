...
import { DATA_START_ROW, fieldColumns, HEADER_ROW, MAIN_SHEET, RECORD_ID_COLUMN, SUMMARY_SHEET, TECHNICAL_HEADER } from "./constants";
...
function setCell(cell: ExcelJS.Cell, field: keyof CompanyRecord, value: CompanyRecord[keyof CompanyRecord]) {
  if (field === "techRoles" || field === "programTypes") {
    cell.value = safeExcelText(joinTags(value as string[]));
    return;
  }
  if (field === "applicationDeadline" || field === "verifiedAt" || field === "contactedAt" || field === "followUpAt") {
    cell.value = dateOnlyToExcelDate(value as string | null);
    cell.numFmt = "yyyy-mm-dd";
    return;
  }
  if (field === "applicationUrl" || field === "primarySourceUrl" || field === "secondarySourceUrl") {
...
export function writeRecordCells(worksheet: ExcelJS.Worksheet, rowNumber: number, record: CompanyRecord, fields?: (keyof CompanyRecord)[]) {
  const keys = fields ?? (Object.keys(fieldColumns) as (keyof CompanyRecord)[]);
  for (const field of keys) setCell(worksheet.getCell(rowNumber, fieldColumns[field]), field, record[field]);
}

function copyRowPresentation(worksheet: ExcelJS.Worksheet, from: number, to: number) {
  const source = worksheet.getRow(from);
  const target = worksheet.getRow(to);
  target.height = source.height;
  for (let column = 1; column <= RECORD_ID_COLUMN; column += 1) {
    const sourceCell = source.getCell(column);
    const targetCell = target.getCell(column);
    targetCell.style = cloneValue(sourceCell.style);
    if (sourceCell.dataValidation?.type) targetCell.dataValidation = cloneValue(sourceCell.dataValidation);
...
function trimDataValidations(worksheet: ExcelJS.Worksheet, lastRow: number) {
  const validations = (worksheet as unknown as { dataValidations: { model: Record<string, ExcelJS.DataValidation> } }).dataValidations.model;
  for (const [address, validation] of Object.entries(validations)) {
    const row = Number(address.match(/\d+$/)?.[0]);
    if (!validation?.type || (row >= DATA_START_ROW && row > lastRow)) delete validations[address];
  }
}

function updateTableRange(worksheet: ExcelJS.Worksheet, lastRow: number) {
  const tables = Object.values((worksheet as unknown as { tables: Record<string, ExcelJS.Table> }).tables);
...
function updateConditionalFormatting(worksheet: ExcelJS.Worksheet, lastRow: number) {
  const rangeEnd = Math.max(lastRow, DATA_START_ROW);
  const items = (worksheet as unknown as { conditionalFormattings: { ref: string; rules: { formulae?: string[] }[] }[] }).conditionalFormattings;
  for (const item of items) {
    if (item.ref.startsWith("I5:I")) item.ref = `I${DATA_START_ROW}:I${rangeEnd}`;
    if (item.ref.startsWith("S5:S")) item.ref = `S${DATA_START_ROW}:S${rangeEnd}`;
    ...
    item.rules.forEach((rule, index) => {
      if (values[index] && rule.formulae?.some((formula) => formula.includes('"undefined"'))) {
        const column = item.ref[0];
        rule.formulae = [`NOT(ISERROR(SEARCH("${values[index]}",${column}${DATA_START_ROW})))`];
      }
    });
  }
}
...
function updateSubtitle(worksheet: ExcelJS.Worksheet, count: number) {
  ...
  if (typeof cell.value === "string") cell.value = cell.value.replace(/\|\s*\d+\s*บริษัท/, `| ${count} บริษัท`);
}

function replaceFormulaLastRow(formula: string, oldLastRow: number, newLastRow: number) {
  return formula.replace(new RegExp(`([A-Z]+${DATA_START_ROW}:[A-Z]+)${oldLastRow}(?!\\d)`, "g"), `$1${newLastRow}`);
}

function updateSummary(workbook: ExcelJS.Workbook, records: CompanyRecord[], oldLastRow: number, newLastRow: number) {
  ...
  summary.eachRow((row) => row.eachCell((cell) => {
    ...
    if (value && typeof value === "object" && "formula" in value && typeof value.formula === "string") {
      cell.value = { formula: replaceFormulaLastRow(value.formula, oldLastRow, newLastRow) };
    }
  }));
  roleCategoryCounts(records).forEach(({ count }, index) => { summary.getCell(5 + index, 5).value = count; });

  for (let row = 19; row <= 24; row += 1) {
    for (let column = 1; column <= 6; column += 1) summary.getCell(row, column).value = null;
  }
  const now = new Date();
  const today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 12));
  const end = new Date(today.getTime() + 14 * 86_400_000);
  const urgent = records
    .filter((record) => record.announcementStatus === "เปิดรับ" && record.applicationDeadline)
    .filter((record) => {
  ...
  urgent.forEach((record, index) => {
    ...
    summary.getCell(row, 4).value = dateOnlyToExcelDate(record.applicationDeadline);
    summary.getCell(row, 4).numFmt = "yyyy-mm-dd";
    summary.getCell(row, 5).value = record.applicationUrl ? { text: record.applicationUrl, hyperlink: record.applicationUrl } : null;
...
export function prepareTechnicalIds(state: WorkbookState) {
  ...
  records.forEach((record) => worksheet.getCell(rowById.get(record.id)!, RECORD_ID_COLUMN).value = record.id);
}

export function preparePatch(state: WorkbookState, nextRecord: CompanyRecord, fields: (keyof CompanyRecord)[]) {
  const rowNumber = state.rowById.get(nextRecord.id);
  if (!rowNumber) throw new AppError("RECORD_NOT_FOUND", "ไม่พบรายการบริษัทนี้", 404, false);
...
export function prepareAdd(state: WorkbookState, record: CompanyRecord) {
  ...
  if (rowNumber > DATA_START_ROW) copyRowPresentation(state.worksheet, rowNumber - 1, rowNumber);
...
export function prepareDelete(state: WorkbookState, id: string, records: CompanyRecord[]) {
  ...
  updateSubtitle(state.worksheet, records.length);
  updateSummary(state.workbook, records, state.lastDataRow, Math.max(newLastRow, DATA_START_ROW));
}
...
export async function saveAndVerify(
  ...
  const temporaryPath = path.join(directory, `.${path.basename(filePath)}.${token}.tmp.xlsx`);
  const rollbackPath = path.join(directory, `.${path.basename(filePath)}.${token}.rollback.xlsx`);
  try {
    ...
    if (checked.records.length !== expectedRecords.length || ids.size !== checked.records.length || (verify && !verify(checked.records))) {
      throw new AppError("WRITE_FAILED", "ตรวจสอบไฟล์ชั่วคราวไม่ผ่าน จึงไม่แก้ไฟล์จริง", 500, true);
    }
    const main = checked.workbook.getWorksheet(MAIN_SHEET)!;
    if (!main.getColumn(RECORD_ID_COLUMN).hidden) throw new AppError("WRITE_FAILED", "คอลัมน์เทคนิคไม่ได้ถูกซ่อน", 500, true);
    ...
    await fs.unlink(rollbackPath).catch(() => { /* A stale rollback copy is safer than reporting a false write failure. */ });
  } catch (error) {
    await Promise.allSettled([fs.unlink(temporaryPath), fs.access(rollbackPath).then(() => fs.rename(rollbackPath, filePath))]);
